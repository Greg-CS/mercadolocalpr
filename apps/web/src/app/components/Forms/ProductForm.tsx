...
const CDNURL =
  "https://ysctqzmvjoqsftxjrvto.supabase.co/storage/v1/object/public/images/";
...
export const ProductForm = ({ user }: { user: any }) => {
  ...
  const [stateCategory, setStateCategory] = useState<CategoryTable[] | null>(
    null,
  );
  ...
  const onSubmit = async (ev: any) => {
    ...
    if (!title || !description || !price || !location || !category) {
      // Add condition for images later
  ...
  return (
    <>
      <form
        onSubmit={onSubmit}
        className="grid m-auto border-2 border-[#3A4F41] rounded-md p-10 bg-white w-[50%]"
      >
        <input
          type="text"
